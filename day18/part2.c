...
/*   snailfish *result=fishvalues[0];
   ...
   for(int a=0;a<input->size;a++){
       for(int b=0;b<input->size;b++){
	       if(a==b) continue;
	       snailfish *onefish = malloc(sizeof(snailfish));
	       fromString(input->data[a],onefish);
	       snailfish *twofish = malloc(sizeof(snailfish));
	       fromString(input->data[b],twofish);
	       snailfish *redfish = addition(onefish,twofish);
	       snailfish *bluefish = reduce(redfish);
	       maxsum=max(maxsum,magnitude(*bluefish));
       }
   }

   printf("max pairwise magnitude %i\n",maxsum);
