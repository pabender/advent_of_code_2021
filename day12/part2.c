...
int findPathsToEnd(cave *start,cave *end){
   int count=0;
   int oldVisitedTwice = visitedSmallTwice;
   //printf("%s %i\n",start->label,start->numNeighbors);
   int startIndex = (getCave("start")->index);
   if(start->visited==1 && start->index == startIndex)
	   return 0;
   //printf("after startindex check\n");
   if(start->visited==1 && start->isSmall==0)
	   if(visitedSmallTwice==1) {
		   //printf("twice %s\n",start->label);
	visitedSmallTwice=0;
	   } else {
	      return 0;
	   }
   //printf("after smallcheck check\n");
   ...
   for(int i=0;i<start->numNeighbors;i++){
	//printf("neighbor %i\n",start->neighbors[i]);
	cave *neighbor=&(caves[start->neighbors[i]]);
	if( (visitedSmallTwice==1 || neighbor->visited==0) || neighbor->isSmall==1) {
	   count = count + findPathsToEnd(&caves[start->neighbors[i]],end);
	   //printf("%i\n",count);
	}
   }
   start->visited--;
   visitedSmallTwice = oldVisitedTwice;
   return count;
}
